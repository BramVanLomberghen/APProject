class SupervisionsController < ApplicationController
  ...
  def index
    @q = Supervision.ransack(params[:q])
    @supervisions = @q.result(distinct: true)
  ...
  def supervision_params
    params.require(:supervision).permit(:start, :end, :supervisor_exams_id, :classroom_id, :exam_period_id)
  end
end
